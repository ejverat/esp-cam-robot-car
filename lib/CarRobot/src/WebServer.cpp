...
esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    if (res == ESP_OK) {
      size_t hlen =
          snprintf((char *)part_buf, 64, WebServer::_STREAM_PART, _jpg_buf_len);
    ...
    if (res == ESP_OK) {
      res = httpd_resp_send_chunk(req, WebServer::_STREAM_BOUNDARY,
                                  strlen(WebServer::_STREAM_BOUNDARY));
    }
    if (fb) {
      esp_camera_fb_return(fb);
      fb = NULL;
